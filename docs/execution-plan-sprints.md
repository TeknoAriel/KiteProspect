...
| Campo | Valor |
|--------|--------|
| **Sprint activo** | **S30 — Seguimiento: iniciar desde ficha** (F1-E12). Ver `docs/decisions/slice-s30-follow-up-start-from-contact.md`. |
| **Inicio (ISO)** | 2026-04-02 |
| **Fin objetivo (ISO)** | +2–3 días (MVP operable; el agente puede cerrar antes si verify está verde). |
| **Objetivo del sprint** | Alta de `FollowUpSequence` desde `/dashboard/contacts/[id]` + historial de intentos en ficha; sin depender solo del seed. |
| **Última verificación agente** | 2026-04-02: `npm run verify` en verde; decisión S30. |
