...
| Campo | Valor |
|--------|--------|
| **Sprint activo** | **Por definir** — último cerrado: **S29** (inbox leído/no leído, `Conversation.lastReadAt`). Ver `docs/decisions/slice-s29-inbox-read-state.md`. |
| **Inicio (ISO)** | — (rotar al abrir el siguiente sprint). |
| **Fin objetivo (ISO)** | — |
| **Objetivo del sprint** | Definir el siguiente incremento con `docs/mvp-phase1-status.md` y `docs/roadmap.md` (p. ej. backlog opcional en `slice-s29-inbox-read-state.md`). |
| **Última verificación agente** | 2026-04-02: `npm run verify` en verde; Hito 5 con S27–S29; decisión S29. |
