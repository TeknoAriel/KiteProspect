...
| **Captura (API)** | `POST /api/contacts/create` con `CAPTURE_API_SECRET` + `accountSlug` (o `accountId`), email o teléfono; validación de campos + rate limit por IP (memoria); evento `lead_captured`; log estructurado `lead_captured` tras éxito (S28). **Meta Lead Ads:** `GET/POST /api/webhooks/meta-leads` + `Integration` `meta_lead_ads` (`slice-f2-mvp-completion.md`). |
