...
| **CRM básico** | Lista y ficha; búsqueda/filtros/paginación; **notas y tareas con edición** (S27); **tareas cerradas recientes** en ficha (S28); reasignación y pausa de seguimiento (`slice-crm-contacts-filters-assignment-followup-pause.md`); **edición etapas** comercial/conversacional en ficha (S25); **ID en CRM externo** (`Contact.externalId`, ficha + `PATCH /api/contacts/{id}/external`, L18). |
| **Inbox** | Lista con filtros (S18), búsqueda y paginación (S19), rango por fecha UTC `from`/`to` en `updatedAt` (S25); **no leído** si hay entrante posterior a `Conversation.lastReadAt` (S29) + hilo con IA y borrador WhatsApp (S12). Filtro por canal incluye email, SMS y Meta Lead (`slice-demo-channel-simulation-ui.md`). |
...
| **Seguimiento (jobs)** | Cron `GET /api/cron/follow-up-due` + `processDueFollowUps`: **WhatsApp** (Meta), **email** (Resend si `RESEND_API_KEY` + `FOLLOW_UP_FROM_EMAIL`; si no, tarea en ficha), **SMS** (Twilio por defecto o **Telnyx** si `SMS_PROVIDER=telnyx`; teléfono + consent `sms`; si no, tarea manual), **Instagram/otros** → tarea manual; **triggers** opcionales en `FollowUpPlan` (score, etapas) F2-E5; ver `slice-follow-up-channels-email-manual.md`, `slice-l16-f3e5-sms-twilio-follow-up.md`, `slice-l20-f3e5-sms-telnyx-provider.md`. |
...
| **Dashboard (F1-E16)** | KPIs + **S33** en `/dashboard` + **S34/L3** en **`/dashboard/reportes`**: nuevos por canal (ventana 7/14/30 días), **cohorte 4×7 días** (F3-E6 L17), embudo conversacional y **comercial**, **SLA**, tareas/seg. activos, **CSV** `?days=`; **filtro por sucursal** `?branchId=` (F3-E4+ L19) (`slice-s23`, `slice-s33`, `slice-s34`, `slice-l3`, `slice-l17-f3e6-operational-cohort-weeks.md`, `slice-l19-f3e4-operational-reports-by-branch.md`). |
| **Auditoría** | `recordAuditEvent` + evento `session_started` en login + evento de seed; UI admin (`/dashboard/audit`). |
| **Captura (API)** | `POST /api/contacts/create` con **Bearer** = `CAPTURE_API_SECRET` global **o** API key por tenant `kp_…` (F3-E2); `accountSlug` o `accountId`; validación + rate limit por IP; `lead_captured` (S28). **`PATCH /api/contacts/{id}/external`**, **`GET /api/contacts/resolve-external`**, **`POST /api/contacts/crm-batch-sync`** (sync inbound por lotes **L22b**) para `externalId` y etapas. **OpenAPI:** `/openapi-capture-v1.yaml` v1.2.3 (`postCrmBatchSync` **L22b**). **Meta Lead Ads:** `GET/POST /api/webhooks/meta-leads` + `Integration` `meta_lead_ads` (`slice-f2-mvp-completion.md`). |
| **Webhooks salientes (F3-E3 / L14 + L24 + L22)** | Suscripciones por cuenta (`WebhookSubscription`); admin en `/dashboard/account/webhooks`; eventos `lead.captured`, `contact.assignment_changed`, `contact.stages_updated`, `follow_up.sequence_started`, `contact.external_id_updated`; firma `X-Kite-Signature: sha256=<HMAC-SHA256 del body>`; `slice-l14-f3e3-public-webhooks.md`, `slice-l24-f3e3-webhooks-stages-followup.md`, `slice-l22-f3e1-crm-bridge-resolve-webhook.md`. |
| **Multi-sucursal (F3-E4 / L15 MVP)** | `Branch` + `Contact.branchId`; admin `/dashboard/account/branches`; filtro y badge en `/dashboard/contacts`; ficha con selector; captura con `branchSlug`; reportes operativos por sucursal `?branchId=` (L19); `slice-l15-f3e4-multi-branch-mvp.md`, `slice-l19-f3e4-operational-reports-by-branch.md`. |
...
**Fase 3 (en curso / backlog):** **F3-E2**–**F3-E6** paso cohorte/export (L17) cerrados; **L18** F3-E1 slice CRM (`externalId` + API); **L19** F3-E4+ reportes por sucursal; **L20** F3-E5+ SMS Telnyx opcional; **L21** F3-E4++ alcance asesor por sucursal (`Advisor.branchId`, UI + API match feedback); **L22** primer paso F3-E1+ (resolve por `externalId` + webhook `contact.external_id_updated`); **L22b** batch sync CRM → Kite (`POST /api/contacts/crm-batch-sync`); ampliaciones F3-E1+ / **F3-E5++** — ver `docs/execution-plan-sprints.md`.

**Producción operativa:** variables en Vercel/hosting, Meta, Resend, Twilio (SMS) e IA siguen en **`docs/manual-actions-required.md`** y **`docs/produccion-checklist-usuario.md`** (primera URL pública, `AUTH_URL`, demo seed vía `build:vercel`, etc.). **L25–L26:** HSTS (solo build Vercel), `robots.txt`, `/.well-known/security.txt`, `/api/health` sin caché + campos `runtime`/`security` (`slice-l25-production-hardening-ii.md`, `slice-l26-production-hardening-iii.md`).

**Entorno dev (Git):** `docs/decisions/github-ssh-windows-dev.md`; checklist humano ítem 13 en `docs/manual-actions-required.md`.
